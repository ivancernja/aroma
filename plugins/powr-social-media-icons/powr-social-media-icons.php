<?php
    ...
    Plugin URI: https://www.powr.io/plugins/social-media-icons
    Description: Grow your social media following with linked icons  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.3
    ...
    */
    ...
    class Powr_Social_Media_Icons extends WP_Widget{
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Social Media Icons.
        </p>
        <p>
    ...
    function powr_social_media_icons_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-social-media-icons' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-social-media-icons' label='$label'></div>";
      }else{
      	"<div class='powr-social-media-icons'></div>";
      }
    }
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_social_media_icons_menu' );
    function powr_social_media_icons_menu() {
      add_menu_page( 'POWr Social Media Icons', 'POWr Social Media Icons', 'manage_options', 'powr-social-media-icons-settings', 'powr_social_media_icons_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_social_media_icons_options() {
      if ( !current_user_can( 'manage_options' ) )  {
        wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
      }
      echo '<iframe id="powr-social-media-icons" src="https://www.powr.io/wp-create/social-media-icons" frameborder="0" width="100%" height="600px" style="position:absolute; top:0; left:0;z-index: 100;"></iframe>';
      echo '<script>';
      echo 'var ht = window.innerHeight; if(document.getElementById("wpadminbar")){ht -= document.getElementById("wpadminbar").offsetHeight;}';
      echo 'var iframe = document.getElementById("powr-social-media-icons"); iframe.style.height = ht+"px"; iframe.height = ht;';
      echo '</script>';
    }
    //Redirecting to landing page when plugin is activated
    register_activation_hook(__FILE__, 'powr_social_media_icons_plugin_activate');
      add_action('admin_init', 'powr_social_media_icons_plugin_redirect');

      function powr_social_media_icons_plugin_activate() {
      add_option('powr_social_media_icons_plugin_do_activation_redirect', true);
      }

      function powr_social_media_icons_plugin_redirect() {
      if (get_option('powr_social_media_icons_plugin_do_activation_redirect', false)) {
          delete_option('powr_social_media_icons_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-social-media-icons-settings' );
          }
       }
      }
